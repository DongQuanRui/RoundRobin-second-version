...
double Models::calculate_power(double time_s, double time_e, double cpu_usage){
    ...
    if (cpu_usage < 0.7)
    {
        ...
        total_price += ((time_start_24 + 1 - time_start) / 1.0) * (0.7 * a + b * (cpu_usage_big - 0.7) * (cpu_usage_big - 0.7)) * price[time_start_24];
        total_price += ((time_end - time_end_24) / 1.0) * (0.7 * a + b * (cpu_usage_big - 0.7) * (cpu_usage_big - 0.7)) * price[time_end_24];
        
        for (int i=int(time_start) + 1; i<=int(time_end); i++) {
            total_price += (0.7 * a + b * (cpu_usage_big - 0.7) * (cpu_usage_big - 0.7)) * price[(i%24)];
        }
